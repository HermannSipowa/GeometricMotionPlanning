...
S_Depth = 3;    % Integration time for PDE
IntTime = 3;
S_Int   = 5000; % Number of points in the time discretization
T_Int   = 2000; % Number of points in the spcial discretization
% dx      = 5E-3;
% x       = 0:dx:1;
...
tspan   = linspace(0,IntTime,T_Int); % discretization of the curve
space   = [0 logspace(-4,log10(S_Depth),S_Int-1)]; % linspace(0,S_Depth,S_Int); % discretization of time
tMin    = -pi/2;
tMax    = pi/2;
rMin    = 3;
rMax    = 10;
...
tStart = tic;
sol = pdepe(m,@mypendulumpde,@mypdexic,@mypdexbc,tspan,space);
tEnd = toc(tStart);
...
%%  Extract Controls
ctrl      = zeros(2,length(tspan));
[p1, dp1] = pdeval(m,tspan,sol(end,:,1),tspan);
[p2, dp2] = pdeval(m,tspan,sol(end,:,2),tspan);
[p3, dp3] = pdeval(m,tspan,sol(end,:,3),tspan);
[p4, dp4] = pdeval(m,tspan,sol(end,:,4),tspan);
...
tic
[~, xstar] = ode113(@(t,X)Dynamics(t,X,ctrl),tspan,X0());
toc
...
xstar = interp1(tspan,xstar,Xmesh);
u1    = interp2(space,tspan,u1,X,Y);
u2    = interp2(space,tspan,u2,X,Y);
