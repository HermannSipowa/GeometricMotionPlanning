...
for ll = 1
    clc
    smax = 5e2; % Need tpo find an analytic way to solve for this value
    % # of grids in t
    tgrids = 1e3;
    % # of grids in s
    sgrids = 1e3;
...
for ll = 1
    ...
    D = diag([Penalty1*ones(1,4) ones(1,3) Penalty2*ones(1,6)]);
    G = (F.')^(-1)*D*F^(-1);
...
%     end
    ...
    for i = 1:13
        subplot(4,4,i)
        plot(tspan,p(i,:) - X_ode45(:,i),'Color',c1)
        grid on
    end
end
...
[U_eci_Target, Xrel, V_sun, beta_chief] = F_CanonBall(ti,X_chief,Target); % SRP force on the Target
...
% Computing SRP acceleration (Cannonball model)
[U_eci_Target, Xrel, V_sun, beta_chief] = F_CanonBall(ti,Xchief,Target); % SRP force on the Target
% F_body = F_CanonBall(ti,r_chasser,Chasser); % SRP force on the Chasser
F_body = F_SPR_FlatPlate(ti, r_chasser, Chasser, q_Chasser);
