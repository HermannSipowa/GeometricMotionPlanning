function [p, dpdx, xfull, xdrift, bufull, budrift, sol, t, x, xnew, cost, p_initial] = AGHF(tmax, xpoints, xpoints_new, tpoints, k, X, Xf, T, N, M)
...
% numerator = diff(p.').';
% denominator = diff(xnew);
% dp = [drift0 numerator./denominator];
...
for i = 1 : xpoints_new
    ...
    Ffull = get_F(p(:,i)); B = Ffull(:,M+1:end);
    % get full control
    bufull(:,i)  = Ffull^(-1) * ( dpdx(:,i) - drift );
    % get actual control
    budrift(:,i) = (B.'*B)\(B.')* ( dpdx(:,i) - drift ); % bufull(N-M+1:end,i);
end
...
xdrift    = Period.*xdrift;
xfull     = Period.*xfull;
X_ode45   = Period.*X_ode45;
