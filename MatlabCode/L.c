...
CASADI_SYMBOL_EXPORT casadi_int L_n_in(void) { return 4;}
...
void mex_L(int resc, mxArray *resv[], int argc, const mxArray *argv[]) {
  ...
  casadi_real* res[1] = {0};
  if (argc>4) mexErrMsgIdAndTxt("Casadi:RuntimeError","Evaluation of \"L\" failed. Too many input arguments (%d, max 4)", argc);
  ...
  if (--argc>=0) arg[1] = casadi_from_mex(argv[1], w+12, casadi_s0, w+27);
  if (--argc>=0) arg[2] = casadi_from_mex(argv[2], w+24, casadi_s1, w+27);
  if (--argc>=0) arg[3] = casadi_from_mex(argv[3], w+25, casadi_s1, w+27);
  --resc;
  res[0] = w+26;
  i = L(arg, res, iw, w+27, 0);
